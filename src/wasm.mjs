...
function lowerKernel(kernel) {
  const code = [], locals = []; let loops = 0, runtimeChecks = 0;
  ...
  const copyContext = ctx => ({ cache: new Map(ctx.cache), indices: new Map(ctx.indices), accumulators: new Map(ctx.accumulators) });
  ...
  load(kernel.result, root); emit(0x0b);
...
export function emitModule(staged) {
  const kernels = staged.kernels, bodies = kernels.map(lowerKernel);
  ...
  return { bytes: Uint8Array.from(pieces.flat()), needsMemory,
    functions: kernels.map((k, i) => ({ name: k.name, loops: bodies[i].loops,
      ...
      runtimeZipChecks: bodies[i].runtimeChecks })) };
}
