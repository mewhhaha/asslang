...
export async function browserBundle({ benchmark = false } = {}) {
  ...
  const specs=[
    ...
    ['jte','src/jte.mjs','const {createChunkViews}=modules.chunkViews;const {createArrayViews}=modules.arrayViews;const {collectOrderings,orderingKeyLeaves}=modules.ordering;const {isSymbolKey,displayRecordKey}=modules.recordKeys;const {intrinsicArities,stageIntrinsic}=modules.intrinsics;const {fail,prune,showType,builtinArities}=modules.frontend;const {flatTypes,isScalarSchema}=modules.abiSchema;','verifyCertificate,schemaOfType,stage'],
    ...
    ['chunkChecks','test/chunk-composition-browser.mjs','','runChunkCompositionBrowserChecks'],
