...
export async function browserBundle({ benchmark = false } = {}) {
  ...
  const specs=[
    ...
    ['reverse','src/reverse.mjs','const {numericLeaves,prepareDifferential}=modules.differential;','reverseVJP'],
    ['intrinsics','src/intrinsics.mjs','const {reverseVJP}=modules.reverse;const {forwardLinearize,reusableLinearize,valueAndGradient,stopGradient}=modules.differential;','intrinsicArities,inferIntrinsic,stageIntrinsic'],
