...
export async function browserBundle({ benchmark = false } = {}) {
  ...
  const specs=[
    ...
    ['wasm','src/wasm.mjs','const {ABI_VERSION,layout,flatTypes}=modules.abiSchema;','uleb,emitModule'],
    ['compiler','src/compiler.mjs','const {CompileError,parse,infer}=modules.frontend;const {stage,verifyCertificate}=modules.jte;const {emitModule}=modules.wasm;','compile,instantiate,CompileError,verifyCertificate'],
    ...
    ['corpus','examples/corpus.mjs','','corpus,baselines,benchmarkArguments,expansionSource'],
    ['benchmark','scripts/benchmark-core.mjs','const {compile}=modules.compiler;const {Arena,prepareCall,createRuntime,createCapability}=modules.abi;const {corpus,baselines,benchmarkArguments,expansionSource}=modules.corpus;','runBenchmarks,quantiles'],
  ];
  let code='const modules={};\n';
  for(const [name,path,imports,exports] of specs)code+=`modules.${name}=(()=>{${imports}\n${await read(path)}\nreturn {${exports}};})();\n`;
  const sources={};for(const e of corpus)if(!Object.hasOwn(sources,e.path))sources[e.path]=await readFile(new URL('../examples/'+e.path,import.meta.url),'utf8');
  ...
  if(benchmark) {
    ...
    code+='const {compile,instantiate}=modules.compiler;const {createRuntime,createCapability}=modules.abi;const {reference}=modules.reference;const {corpus}=modules.corpus;\n';
